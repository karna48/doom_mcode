classdef t_factory
    methods (Static = true)
        ...
        function r = mobjinfo_t(doomednum, spawnstate, spawnhealth, seestate, seesound,
            ...
            r = struct('doomednum', int32(doomednum),
            'spawnstate', int32(spawnstate),
            'spawnhealth', int32(spawnhealth),
            'seestate', int32(seestate),
            'seesound', int32(seesound),
            'reactiontime', int32(reactiontime),
            'attacksound', int32(attacksound),
            'painstate', int32(painstate),
            'painchance', int32(painchance),
            'painsound', int32(painsound),
            'meleestate', int32(meleestate),
            'missilestate', int32(missilestate),
            'deathstate', int32(deathstate),
            'xdeathstate', int32(xdeathstate),
            'deathsound', int32(deathsound),
            'speed', int32(speed),
            'radius', int32(radius),
            'height', int32(height),
            'mass', int32(mass),
            'damage', int32(damage),
            'activesound', int32(activesound),
            'flags', int32(flags),
